...
const EXPORT_COLUMNS = [
  ...
  { key: 'salary', label: 'Salary', width: 52 },
];
...
function cellValue(row, key) {
  ...
  if (key === 'salary') {
    if (row.salary == null || row.salary === '') return '';
    const n = Number(row.salary);
    return Number.isFinite(n) ? String(n) : String(row.salary);
  }
  const value = row[key];
  ...
  const body = rows
    .map((row, index) => {
      ...
      const cells = EXPORT_COLUMNS.map((col) => {
        ...
        const number = col.key === 'salary' && value !== '' && Number.isFinite(Number(value));
        if (number) {
          return `<Cell${style}><Data ss:Type="Number">${xmlEscape(value)}</Data></Cell>`;
        }
...
async function buildEmployeePdf(records, meta = {}) {
  ...
  function addPage() {
    ...
    safeDrawText(page, `Confidential employee directory`, {
